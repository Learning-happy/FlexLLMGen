...
def foward_trace(recv_queue:Queue, recv_lock,
    ...
    while True: # 持续处理，直到父进程通知其结束
        ...
        send_lock.acquire()
        send_queue.put(["e",session_id,pid,stored_tokens,session_active_index,finish_time,R_time,W_time])
        send_lock.release()
...
def run__process():
    ...
    while session_active_num != 0:
        ...
        if SEND_FLAG: # 派发新任务
            send_lock.acquire()
            send_queue.put([session_id,qid,session_stored_tokens[session_id],session_active_index])
            send_lock.release()
